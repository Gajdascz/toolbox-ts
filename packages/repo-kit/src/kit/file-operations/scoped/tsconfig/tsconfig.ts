...
import { merge as _merge, type TsConfigs } from '@toolbox-ts/configs/core';
import { isFile, mergeJSON } from '@toolbox-ts/file';
import fs from 'node:fs/promises';
import path from 'node:path';

import type { ScopedFileMergeOp } from '../types.ts';

import { OperationError } from '../../../../../Errors.js';
export interface TsConfigOps<T extends string = TsConfigs.TsConfigDomain> {
...
export const createTsConfigOps = async <T extends TsConfigs.TsConfigDomain>(
  type: T,
  cwd: string,
  filename: string
): Promise<TsConfigOps> => {
