...
const normalizePaths = (
  pathStrings: (string | string[] | undefined | null | false)[]
): string[] => {
  const result: string[] = [];
...
export const createForbiddenRule = <N extends string>(
  ...
  generate: ({
    ...
    const { path: fromInputPath, pathNot: fromInputPathNot, ...restFromInput } = fromInput;
    const {
      path: fromDefaultsPath,
      pathNot: fromDefaultsPathNot,
      ...restFromDefaults
    } = fromDefaults;
    const { path: toInputPath, pathNot: toInputPathNot, ...restToInput } = toInput;
    const { path: toDefaultsPath, pathNot: toDefaultsPathNot, ...restToDefaults } = toDefaults;
