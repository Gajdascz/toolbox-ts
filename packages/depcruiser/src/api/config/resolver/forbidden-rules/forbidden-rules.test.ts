...
import { resolveForbiddenRules } from './forbidden-rules.ts';
...
it('should return an array of rules with default configs', () => {
  const result = resolveForbiddenRules({});
...
it('should return an array of rules with some omitted', () => {
  ...
  const result = resolveForbiddenRules(setToFalse);
  expect(Array.isArray(result)).toBe(true);
...
it('should return an array of rules with some extended', () => {
  ...
  const result = resolveForbiddenRules({}, mockExtendedRules);
...
it('should use default config when set to true', () => {
  const result = resolveForbiddenRules({ noCircular: true });
