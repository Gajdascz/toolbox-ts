...
import type { OrchestratorConfigEntry } from '../types.ts';

import { $shell, mapConfigModuleDeps } from '../../../project/index.js';
...
export const get = ({
  ...
  if (usingChangeset) dependencies.push(['@changesets/cli', { isDev: true }]);
  return {
    dependencies,
    postProcess: async () => {
      ...
      if (usingChangeset) await $shell('pnpm exec changeset init');
    },
    pkgPatch: {
      ...lsPkgPatch,
      ...(includeCommitizen && {
