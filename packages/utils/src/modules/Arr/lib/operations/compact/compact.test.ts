...
import { compact, compactFalsy, compactNull, compactUndefined } from './compact.ts';
...
describe('Array Compact', () => {
  ...
  describe('Falsy', () => {
    it('removes falsy from mixed array', () => {
      expect(compactFalsy([0, 1, false, 2, '', 3, null, undefined, 4, Number.NaN])).toEqual([
        1, 2, 3, 4
      ]);
    });
  ...
  it('Null', () => {
    expect(compactNull([1, null, 2, null, 3, undefined])).toEqual([1, 2, 3, undefined]);
    expect(compactNull([null, null])).toEqual([]);
  });
  it('Undefined', () => {
    expect(compactUndefined([1, undefined, 2, undefined, 3])).toEqual([1, 2, 3]);
