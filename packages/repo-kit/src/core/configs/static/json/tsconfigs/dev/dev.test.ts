...
describe('dev.ts config module', () => {
  describe('define', () => {
    it('should create config with static fields when called with no arguments', () => {
      ...
      expect(config).toMatchObject({
        ...STATIC_FIELDS,
        compilerOptions: STATIC_COMPILER_OPTIONS,
        exclude: EXCLUDE,
        include: INCLUDE
      });
    });
    ...
    it('should append custom exclude patterns to static excludes', () => {
      ...
      expect(config.exclude).toEqual([...customExclude, ...EXCLUDE]);
    });
    ...
    it('should append custom include patterns to static includes', () => {
      ...
      expect(config.include).toEqual([...customInclude, ...INCLUDE]);
    });
    ...
    it('should merge all custom fields with static fields', () => {
      ...
      expect(config).toMatchObject({
        ...
        typeAcquisition: { enable: true },
        watchOptions: { watchFile: 'usefsevents' },
        compilerOptions: { strict: true, ...STATIC_COMPILER_OPTIONS },
        exclude: ['custom-exclude/**', ...EXCLUDE],
        include: ['custom-include/**', ...INCLUDE]
      });
    });
  ...
  describe('integration', () => {
    it('should create, define, and format a complete config workflow', () => {
      ...
      const expectedInclude = [...additionalInclude, ...INCLUDE];
      const config = define({
