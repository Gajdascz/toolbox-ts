...
runSyncAsync('File.Find.tryAllUp()', { async: tryAllUp, sync: tryAllUpSync }, [
  {
    itShould: 'find all matching files',
    case: () => ({
      fnArgs: ['**/*.txt', { startDir: L4 }] as const,
      assertion: 'toMatchObject',
      expected: { ok: true },
      after: async () => {
        const res: any = await tryAllUp('**/*.txt', { startDir: L4 });
...
runSyncAsync('File.Find.tryFirstUp()', { async: tryFirstUp, sync: tryFirstUpSync }, [
  {
...
runSyncAsync('File.Find.tryLastUp()', { async: tryLastUp, sync: tryLastUpSync }, [
  {
