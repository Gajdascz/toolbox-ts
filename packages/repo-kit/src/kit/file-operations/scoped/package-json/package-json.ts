...
import { merge as _merge, PACKAGE_JSON_FILE } from '@toolbox-ts/configs/core';
...
import { $pnpm } from '../../../../../../architecture/shell-exec.js';
...
export const createPackageJsonOps = async (
  ...
  const execOpts: Options = { ...opts, cwd: dir };
  const pkgPath =
    dir.endsWith(PACKAGE_JSON_FILE) ? dir : (
      path.join(dir, packageJson.meta.filename)
    );
