...
const resolveAffected = (affected?: HandleAffectedOptions): IFormatOptions => {
  ...
  if (typeof affected === 'string') return { ...AFFECTED_DEFAULTS, oldRevision: affected };
  const { extensions: defaultExtensions = '', ...restDefaults } = AFFECTED_DEFAULTS;
