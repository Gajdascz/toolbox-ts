...
export const isMocked = (module: Record<string | symbol, unknown>): boolean =>
  ...
  (!module.default ||
    (typeof module.default === 'object' &&
      ...
      (module.default as Record<string | symbol, unknown>)[MOCKED_MODULE] === true));
...
export const assertMockedEnv = async (mockChecks: MockCheck[] = []): Promise<void> => {
  const errors: string[] = [];
  for (const { validate, description, modulePath } of mockChecks) {
    try {
      const module = (await import(modulePath)) as Record<string | symbol, unknown>;
      /* c8 ignore start */
...
export type MockedModule<M> = {
  [K in keyof M]: M[K] extends (...arguments_: unknown[]) => unknown
    ? MockedFn
...
export function recursiveReset(object: Record<string, unknown>, seen = new WeakSet()): void {
  if (seen.has(object)) return;
  seen.add(object);
  for (const value of Object.values(object)) {
    if (typeof value === 'function' && 'mockReset' in value) (value as MockedFn).mockReset();
...
export const dependencyExists = async (importPath: string, log = false) => {
  try {
    ...
    if (log)
      console.warn(`Failed to import ${importPath}. Make sure it's an installed dependency.`);
    return false;
  }
};
