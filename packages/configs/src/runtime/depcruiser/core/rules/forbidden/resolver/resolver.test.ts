...
it('should return an array of rules with default configs', () => {
  ...
  expect(result.every(({ name }) => name && NAMES.has(name))).toBe(true);
});
...
it('should return an array of rules with some omitted', () => {
  ...
  expect(result.every(({ name }) => name && !(name in setToFalse))).toEqual(true);
});
it('should return an array of rules with some extended', () => {
  ...
  expect(result.every(({ name }) => name && withExtended.has(name))).toEqual(true);
  expect(result.find((r) => r.name === 'mock-rule-1')).toEqual(mockExtendedRules[0]);
  expect(result.find((r) => r.name === 'mock-rule-2')).toEqual(mockExtendedRules[1]);
});
